...
public class CheckInController implements Initializable {
    ...
    public void setInitialData(int idSettlement, int idClient, Rooms initialRoom, LocalDate arrivalDate, LocalDate departureDate, String paymentStatus) {
        ...
        if (paymentStatus != null && cmbPaymentStatus != null) cmbPaymentStatus.setValue(paymentStatus);
        ...
        if (initialRoom != null) {
            ...
            if (!alreadyExists) cmbRoom.getItems().add(initialRoom);
    ...
    private void updateAvailableRoomsList() {
        ...
        try {
            ...
            if (this.initialRoomRef != null) {
                ...
                if (!containsInitial) roomOptions.add(this.initialRoomRef);
            }
    ...
    private void handleSave() {
        ...
        if (!settlementService.isCheckInDateValid(arrivalDate)) {
            alerts.showError("Помилка", "Пряме поселення можливе лише на сьогодні!");
            return;
        }

        if (dateLeaving == null || selectedClient == null || selectedRoom == null || paymentStatus == null) {
        ...
        if (!settlementService.isRoomAvailable(selectedRoom, arrivalDate, dateLeaving, currentSettlementId, initialRoomRef)) {
            String msg = currentSettlementId > 0
                    ? "На вибрані дати цей номер уже зайнятий іншим гостем!"
                    : "Неможливо оформити поселення. Цей номер зайнятий на вказані дати!";
            alerts.showError("Номер зайнято", msg);
            return;
        }

        try {
            SettlementService.SettlementSaveResult result = settlementService.saveSettlement(
                    currentSettlementId, selectedClient.getIdClient(),
                    selectedRoom, arrivalDate, dateLeaving, paymentStatus
            );
