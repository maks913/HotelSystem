...
public class ChessController implements Initializable {
    ...
    private void refreshDataAndBuildGrid() {
        try {
            ...
            currentBookings = bookingDAO.findAll();
            currentSettlements = settlementDAO.findAll();
            buildGrid();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private Pane createCell(Rooms room, LocalDate date) {
        ...
        String status = getRoomStatusOnDate(room, date);
    ...
    /** Визначення статусу кімнати на конкретну дату. */
    private String getRoomStatusOnDate(Rooms room, LocalDate date) {
        if ("Ремонт".equalsIgnoreCase(room.getStatus())) {
            return "Ремонт";
        }

        if (currentSettlements != null) {
            for (Settlements s : currentSettlements) {
        ...
        if (currentBookings != null) {
            for (Bookings b : currentBookings) {
    ...
    private void handlePrintFinancialReport() {
        ...
        try {
            ...
            String reportText = Reports.buildFinancialReport(selectedMonth, yearNum, sData, bData);
    ...
    private void handlePrintOperationalReport() {
        ...
        try {
            Object[] opStats = settlementDAO.getOperationalStats(monthNum, yearNum);
